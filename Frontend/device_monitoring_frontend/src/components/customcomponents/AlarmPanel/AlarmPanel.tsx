...
const AlarmPanel = ({
  ...
  const [shouldConnectSignalR, setShouldConnectSignalR] =
    useState<boolean>(true);
  const [currentExpandedUnackAlarm, setCurrentExpandedUnackAlarm] =
    useState<string>("");
  const [currentExpandedAckAlarm, setCurrentExpandedAckAlarm] =
    useState<string>("");
  ...
  function filterAndSortAlarms(data: any) {
    const investigatingAlarms = sortAlarmsDataBySeverity(
      data.filter(
        (alarm: any) =>
          alarm.isAcknowledged && alarm.alarmState == "Investigating"
      )
    );
    const resolvedAlarms = sortAlarmsDataBySeverity(
      data.filter(
        (alarm: any) => alarm.isAcknowledged && alarm.alarmState == "Resolved"
      )
    );
    const ignoredAlarms = sortAlarmsDataBySeverity(
      data.filter(
        (alarm: any) => alarm.isAcknowledged && alarm.alarmState == "Ignored"
      )
    );
    ...
    setUnacknowledgedAlarms(
      sortAlarmsDataBySeverity(
        data.filter((alarm: any) => !alarm.isAcknowledged)
      )
    );
  }
  ...
  const fetchData = async (selectedDevices: any[], dateRange: any) => {
    const response = await getAlarmPanelData({
  ...
  useEffect(() => {
    if (devicesNameMacList) setDevices(devicesNameMacList);
  }, []);

  useEffect(() => {
    if (!selectedDevicePropertyPanel) {
      fetchData(
        selectedDevices.map((s: any) => s.deviceMacId),
        dateRange
      );
      selectedDevices.length == 0 && dateRange == null
        ? setShouldConnectSignalR(true)
        : setShouldConnectSignalR(false);
    }
  ...
  const investigateAlarmData = async (alarmId: any) => {
    ...
    if (response && response.data) {
      ...
      setUnacknowledgedAlarms((prev: any) =>
        prev.filter((a: any) => a.id != alarmId)
      );
  ...
  const removeunacknowledgedAlarm = async (alarmId: any, input: any) => {
    ...
    if (response && response.data) {
      ...
      ackAlarm.alarmComment = response.data.alarmComment;
      ackAlarm.acknowledgedFrom = response.data.acknowledgedFrom;
      const newUnackList = unacknowledgedAlarms.filter(
        (a: any) => a.id != alarmId
      );
    ...
    <div className={styles.panel}>
        ...
        <div className={styles.header}>
          <h2>
            ...
            <span className={styles.count}>
              <Badge
                label={(
                  unacknowledgedAlarms.length + acknowledgedAlarms.length
                ).toString()}
                bgColor="neutral"
                textColor="dark"
              />
          ...
          {(selectedDevices.length > 0 || dateRange != null) && (
            <Tooltip
              openDelay={100}
              closeDelay={150}
              content={<span className="p-2">Clear alarm filter</span>}
            >
              <ListX
                className={styles.clearAlarmFilterIcon}
                onClick={() => {
                  clearAlarmFilter();
                }}
                strokeWidth={"2.5px"}
      ...
      <div className={`${styles.downContainer}`}>
        <div className={`${styles.section}`}>
          <Accordion
            title={
              <h3 className={styles.alarmPanelTitles}>
                Unacknowledged{" "}
                <span className={styles.sectionCount}>
                  <Badge
                    label={unacknowledgedAlarms.length.toString()}
                    bgColor="neutral"
                    textColor="dark"
                  />
                </span>
              ...
              {unacknowledgedAlarms.map((alarm) => {
                return (
                  <div className={`${styles.alarmCard}`} key={alarm.id}>
                    <AlarmCard
                      key={alarm.id}
                      removeunacknowledgedAlarm={removeunacknowledgedAlarm}
                      setCurrentExpandedUnackAlarm={
                        setCurrentExpandedUnackAlarm
                      }
                      currentExpandedUnackAlarm={currentExpandedUnackAlarm}
                      alarm={alarm}
                      investigateAlarm={investigateAlarmData}
                      resolveAlarm={resolveAlarmData}
                    />
                  </div>
                );
              })}
          ...
          <Accordion
            title={
              <h3 className={styles.alarmPanelTitles}>
                ...
                <span className={styles.sectionCount}>
                  <Badge
                    label={acknowledgedAlarms.length.toString()}
                    bgColor="neutral"
                    textColor="dark"
                  />
                </span>
            ...
            <div className={`${styles.alarmsAccordionSection} `}>
              {acknowledgedAlarms.map((alarm) => {
                return (
                  <div className={`${styles.alarmCard}`} key={alarm.id}>
                    <AlarmCard
                      removeacknowledgedAlarm={removeacknowledgedAlarm}
                      setCurrentExpandedAckAlarm={setCurrentExpandedAckAlarm}
                      currentExpandedAckAlarm={currentExpandedAckAlarm}
                      key={alarm.id}
                      alarm={alarm}
                      investigateAlarm={investigateAlarmData}
                      resolveAlarm={resolveAlarmData}
                      resolveInvestigatedAlarmData={
                        resolveInvestigatedAlarmData
                      }
                    />
                  </div>
